...
z/OS est l'équivalent de Linux ou Windows, mais pour les Mainframes IBM. Il est conçu pour garantir une disponibilité quasi permanente (99,999 % de disponibilité).

### 2.2 COBOL 6.5 : le langage qui lit les données

COBOL est un langage de programmation. Il est massivement utilisé sur tous les Mainframe et celui de LCL ne fait pas exception avec plus de 85% de son parc en COBOL.

La version 6.5 apporte des améliorations de performance et des capacités de connectivité modernes.

La verison 6.4 minimum est requise pour utiliser le Kafka SDK for z/OS.
...
Sur Mainframe, les technologies historiques comme DB2 (base de données) et MQ (messagerie) offrent nativement un mécanisme de rollback : si le programme plante à 99 % de sa progression, tout ce qui a été fait est automatiquement annulé, comme si rien ne s'était passé.

Sur Apache Kafka, ce mécanisme n'existe pas nativement. Kafka est conçu pour la performance et la scalabilité, pas pour la gestion transactionnelle au sens bancaire. Les messages envoyés à Kafka sont persistés immédiatement et ne peuvent pas être "annulés" facilement.
...
## 5. Les 5 stratégies de sécurisation
...
!!! warning "Inconvénients" - Développement COBOL plus complexe : il faut gérer la logique de reprise. - Risque de doublons si le checkpoint est écrit avant la confirmation Kafka (à combiner avec l'idempotence, voir section 5.3).
...
### 5.4 Le Post-Step TWS avec Signal de Fin (Solution recommandée pour le batch)
...
Le pilotage du Job A (Publication Kafka) utilise une **exit propriétaire** et un mécanisme de **recovery automatique** :
...
3.  **Scénario de Succès ($RC \le 7$) :** TWS lance un step de notification positive. Un traitement batch envoie un signal de "Complétude" à l'application Open via **z/OS Connect**.
...
L'exit propriétaire arrête le job et TWS bloque le pipeline. L'équipe de supervision est alertée. Grâce au step de notification d'erreur, le monde Open est activement prévenu de l'invalidité des données et le **Job B** (consommation) n'est jamais déclenché.

!!! success "Avantages" - **Sécurité de bout en bout :** L'application Open ne traite jamais de données partielles grâce à la notification explicite d'abandon. - **Automatisme de reprise :** Le mécanisme de recovery TWS permet une gestion propre des erreurs sans intervention manuelle systématique pour informer l'aval. - **Lisibilité opérationnelle :** Les équipes de supervision voient immédiatement si le traitement est en succès ou en phase de "Recovery/Notification". - **Intégration moderne :** L'utilisation de **z/OS Connect** permet une communication fluide et synchrone entre le Mainframe et les API du monde Open.
...
### 5.5 Le Pattern File MQ + Kafka Connect

#### Principe

Ce pattern place IBM MQ comme **tampon transactionnel** entre le programme COBOL et Kafka. Un connecteur Kafka Connect surveille la file MQ et publie les messages vers Kafka au fil de leur validation par le programme.

!!! warning "Ce pattern n'est pas du streaming pour un batch de 21 Go"
L'appellation "streaming" s'applique aux sources **événementielles** (CICS, IMS) où chaque transaction est validée individuellement et immédiatement. Dans le contexte d'un batch COBOL traitant 21 Go, le comportement réel dépend du découpage en syncpoints — aucun flux continu en temps réel n'est possible.

!!! tip "Analogie"
Si le check-pointing est un camion qui charge par palettes et valide chaque palette avant de repartir, ce pattern ajoute un quai intermédiaire (MQ) entre le camion et la destination : chaque palette validée part immédiatement vers Kafka, sans attendre les suivantes.

---

#### Le Cœur du Réacteur : La Gestion du Rollback et de la Complétude

IBM MQ garantit l'intégrité des données grâce au mécanisme de **Syncpoint (Unité de Travail)**.

##### 1. Pas de file temporaire, mais une "visibilité différée"

- **L'état "Uncommitted" :** Quand le programme COBOL écrit (`MQPUT`) sous syncpoint, le message est physiquement écrit dans la file et dans les logs de récupération MQ, mais marqué comme **invisible**.
- **Côté Kafka Connect :** Le connecteur ne peut lire (`MQGET`) que les messages dont le statut est **"Committed"**. Tant que le syncpoint est ouvert, le connecteur ne voit rien.

##### 2. Le scénario de plantage (Rollback)

Si le programme COBOL subit un **Abend** ou rencontre une erreur avant de valider le syncpoint en cours :

- Le gestionnaire MQ détecte la rupture de l'unité de travail.
- Il annule les messages non commités en consultant ses logs : ils sont supprimés de la file.
- **Résultat :** Kafka ne reçoit aucune donnée partielle pour le bloc en cours.

##### 3. Syncpoint unique vs syncpoints multiples : le compromis fondamental

!!! danger "Syncpoint unique sur 21 Go — non viable en production"
Un syncpoint couvrant la totalité des 21 Go maintient tous les messages en état "Uncommitted" dans les logs MQ pendant toute la durée du traitement. En pratique :

    - Les logs MQ ont des limites de taille configurables — un dépassement arrête le programme.
    - 21 Go de données immobilisées représentent une pression importante sur le stockage z/OS.
    - Le consommateur AWS attend la fin du traitement entier avant de recevoir quoi que ce soit : le comportement est identique à un batch classique.
    - **Un syncpoint unique sur 21 Go est rarement viable en production.**

La solution est d'utiliser **plusieurs syncpoints** — un par bloc traité, de la même façon que le check-pointing (section 5.1) :

| Mode                 | Atomicité                                | Comportement côté Kafka       | Faisabilité pour 21 Go       |
| -------------------- | ---------------------------------------- | ----------------------------- | ---------------------------- |
| Syncpoint unique     | Totale — rollback complet en cas d'abend | Batch pur : rien avant la fin | Non viable (logs MQ saturés) |
| Syncpoints multiples | Partielle — bloc par bloc                | Progression par blocs         | Recommandée                  |

Avec plusieurs syncpoints, des **données partielles peuvent atteindre Kafka** entre deux commits. Le consommateur doit donc attendre un **signal de complétude** avant de traiter (voir section 5.4 — Post-Step TWS).

##### 4. La validation (Commit) et le transfert

Après chaque `MQCMIT` (validation d'un bloc) :

- MQ change le statut des messages du bloc en **"Committed"**.
- Ils deviennent visibles pour Kafka Connect qui les aspire immédiatement.
- Le connecteur ne confirme la lecture à MQ qu'après avoir reçu l'ACK de Kafka, garantissant une livraison **"at-least-once"**.

!!! warning "At-least-once implique des doublons potentiels"
En cas de panne entre l'envoi Kafka et l'acquittement à MQ, un message peut être publié deux fois. Le consommateur doit être **idempotent**, ou ce pattern doit être combiné avec l'idempotence Kafka (section 5.3).

---

#### Quand ce pattern est-il réellement pertinent ?

| Source                             | Comportement réel                                        | Latence vers Kafka         |
| ---------------------------------- | -------------------------------------------------------- | -------------------------- |
| CICS (transaction individuelle)    | Vrai streaming — chaque transaction = un commit immédiat | Quelques millisecondes     |
| IMS Message-Driven                 | Vrai streaming — idem                                    | Quelques millisecondes     |
| Batch COBOL — syncpoints multiples | Progression par blocs                                    | Quelques secondes par bloc |
| Batch COBOL — syncpoint unique     | Batch pur — rien avant la fin                            | Plusieurs heures           |

Pour ce projet (batch 21 Go), le pattern est applicable **uniquement avec des syncpoints multiples**, couplé au signal de fin de la section 5.4.

---

#### Fonctionnement Technique Global

1. **Production (Mainframe) :** Le programme COBOL écrit dans la file MQ par blocs, sous syncpoints multiples.
2. **Ingestion (Kafka Connect) :** Le **IBM MQ Source Connector** maintient une connexion persistante avec MQ et consomme les messages au fil des commits.
3. **Transformation (SMT & Converters) :** Le connecteur traduit le format binaire propriétaire (EBCDIC, COMP-3 via Copybook) en JSON ou Avro grâce à un Schema Registry.
4. **Consommation (AWS) :** Les applications consomment les données après réception du signal de complétude, avec la garantie que chaque message appartient à un bloc Mainframe validé.

#### Pourquoi choisir ce pattern ?

- **Découplage fort :** Le Mainframe pousse ses données sans que les applications AWS accèdent directement aux bases z/OS (DB2, VSAM).
- **Résilience par bloc :** En cas d'abend entre deux syncpoints, seul le bloc en cours est perdu — les blocs précédents sont déjà dans Kafka.
- **Scalabilité :** MQ absorbe les pics de production ; Kafka Connect régule le débit vers Kafka.

!!! warning "Inconvénients" - Complexité accrue côté COBOL : gestion des syncpoints multiples et de la logique de blocs. - Complexité côté consommateur : doit attendre le signal de fin avant de traiter (section 5.4). - Infrastructure MQ supplémentaire à opérer et maintenir sur z/OS.

```mermaid
---
title: Pattern File MQ + Kafka Connect — Flux par blocs (syncpoints multiples)
---
%%{init: {"theme": "base", "themeVariables": {"background": "#ffffff"}}}%%
flowchart LR
    classDef startStop fill:#e1f5fe,stroke:#01579b,color:#000
    ...
    subgraph MF["Mainframe z/OS"]
        direction TB
        APP[Programme COBOL]:::logic
        MQ[/File IBM MQ — queue/]:::data
    end

    subgraph BRIDGE["Zone de transit"]
        direction TB
        KC[[Kafka Connect — IBM MQ Source Connector]]:::external
    end
    ...
    APP -->|MQPUT — blocs sous syncpoint| MQ
    APP -->|MQCMIT — valide chaque bloc| MQ
    MQ -->|lecture des messages committés| KC
    KC -->|publie dans le topic| KK
    KK -->|consomme les données| CONS
