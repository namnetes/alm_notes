...
utilisateurs **Windows** (y compris WSL) et **Linux**.

!!! info "Vous êtes sur macOS ?"
    Plusieurs raccourcis utilisent une touche différente sur Mac (`Option` au
    lieu d'`Alt`, `Cmd` au lieu de `Ctrl` pour certaines actions) et
    nécessitent parfois une configuration additionnelle du terminal. Consultez
    la page dédiée : [Raccourcis clavier — macOS](raccourcis-macos.md).

!!! info "Convention de notation"
    Les raccourcis sont notés avec `+` entre les touches : ++ctrl+c++
    signifie « maintenir Ctrl et appuyer sur C ». Une séquence comme
...
| ++ctrl+v++ (terminaux Linux) ou ++alt+v++ (Windows/WSL) | Coller une image du presse-papiers — insère une puce `[Image #N]` réutilisable dans le prompt. Sous WSL, `Ctrl+V` **et** `Alt+V` sont tous les deux actifs : utilisez `Alt+V` si votre terminal intercepte `Ctrl+V` |
...
| ++shift+tab++ ou ++alt+m++ | Faire défiler les **modes de permission** : `default`, `acceptEdits`, `plan`, et tout mode personnalisé activé (`auto`, `bypassPermissions`…) |
| ++alt+p++ | Changer de modèle, sans effacer le prompt en cours |
| ++alt+t++ | Activer/désactiver la réflexion étendue (*extended thinking*) — sans effet sur Fable 5, qui l'utilise toujours |
| ++alt+o++ | Activer/désactiver le mode rapide (*fast mode*) |
...
| ++ctrl+u++ | Supprimer du curseur jusqu'au début de la ligne (répéter pour effacer plusieurs lignes en saisie multiligne) |
| ++ctrl+w++ | Supprimer le mot précédent. Sous Windows, `Ctrl+Retour arrière` fait aussi l'affaire |
| ++ctrl+y++ | Recoller le dernier texte supprimé par ++ctrl+k++, ++ctrl+u++ ou ++ctrl+w++ |
| ++alt+y++ (juste après ++ctrl+y++) | Faire défiler l'historique des suppressions pour recoller un extrait plus ancien |
...
| Shift+Entrée | ++shift+enter++ | Natif dans WezTerm, Ghostty, Kitty, Warp, Windows Terminal |
...
| `/`  (dans un chemin) | Déclenche une autocomplétion de chemin de fichier en direct (utiliser `/` même sous Windows) |
...
| ++open-bracket++ | Écrire la conversation complète dans le scrollback natif du terminal (nécessite le rendu plein écran) |
...
| Raccourci | Effet |
|---|---|
| `Ctrl+clic` sur le lien | Ouvrir la pull request dans le navigateur |
