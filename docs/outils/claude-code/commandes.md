...
!!! info "Commandes et skills sont aujourd'hui le même mécanisme"
    Depuis la fusion de 2026, un fichier `.claude/commands/x.md` et un
    [skill](skills.md) `.claude/skills/x/SKILL.md` créent tous les deux la
    commande `/x` et fonctionnent pareil : invocables à la main **et**
    automatiquement par Claude par défaut. `commands/` reste le format
    simple (un seul fichier, sans ressources associées) ; `skills/` est
    recommandé dès que vous voulez joindre des scripts/gabarits, ou
    contrôler finement qui a le droit d'invoquer (`disable-model-invocation`,
    `user-invocable` — voir [Skills](skills.md#controler-qui-invoque-le-skill)).
    En cas de conflit de nom, le skill l'emporte sur la commande.
...
!!! info "Sur cette machine (juillet 2026)"
    Une seule commande personnelle est déployée : **`/optimise-doc`**
    (relecture et amélioration d'une documentation MkDocs Material —
    admonitions, onglets, Mermaid, grilles de cartes). Elle est versionnée
    dans `alm_dots/.claude/commands/optimise-doc.md` et stowée vers
    `~/.claude/commands/optimise-doc.md`.
