...
Cette page est la **référence de la configuration**. Les workflows et
exemples d'usage au quotidien sont dans
**[Tirer le meilleur parti de Kitty](guide.md)**.
...
Kitty est installé depuis les **binaires officiels upstream**, pas par
apt — le paquet Ubuntu Noble est figé en 0.32.x, alors que l'upstream
avance vite (0.47+). Tout vit dans le HOME :

```bash
# Via le post-install alm_tools (module apps/install_kitty.sh) :
sudo make -C ~/alm_tools/postinstall kitty

# Ou à la main — c'est ce que fait le module :
curl -L https://sw.kovidgoyal.net/kitty/installer.sh | sh /dev/stdin launch=n
ln -sf ~/.local/kitty.app/bin/kitty ~/.local/bin/kitty
ln -sf ~/.local/kitty.app/bin/kitten ~/.local/bin/kitten
```

- **Binaires** : `~/.local/kitty.app/` ; symlinks dans `~/.local/bin`
  (en tête de `$PATH`).
- **Lanceur GNOME** : `kitty.desktop` + `kitty-open.desktop` copiés dans
  `~/.local/share/applications/` avec `Exec=`/`Icon=` absolus — le
  favori du dock continue de fonctionner.
- **Terminfo** : le paquet apt `kitty-terminfo` reste installé
  (via `packages_to_install.list`) pour que les shells root et les
  autres comptes connaissent `xterm-kitty`.
- **Mise à jour** : relancer la commande `curl … installer.sh` —
  elle écrase `kitty.app` en place (pas de mise à jour apt).
...
| Raccourci | Action |
|-----------|--------|
| `Ctrl+1` … `Ctrl+5` | Aller à l'onglet 1 … 5 (numéro affiché dans l'onglet) |
| `kitty_mod+(` | Fenêtre précédente (panneau) |
| `kitty_mod+)` | Fenêtre suivante (panneau) |
| `kitty_mod+k` / `kitty_mod+j` | Prompt précédent / suivant dans le scrollback |
| `kitty_mod+z` | Basculer le layout Stack (plein écran d'un panneau) |
| `kitty_mod+d` | Nouvelle fenêtre en split, dans le répertoire courant |
| `kitty_mod+Entrée` | Nouvelle fenêtre dans le répertoire courant |
| `kitty_mod+t` | Nouvel onglet dans le répertoire courant |
...
| `kitty_mod+F12` | Redémarrer `mkdocs.service` (overlay de confirmation) |
| `kitty_mod+F1` | Nouvel onglet → launcher SSH interactif (VMs KVM) |
| `kitty_mod+F9` | Broadcast : taper dans toutes les fenêtres à la fois |

### Clipboard, markers et défauts conservés

| Raccourci | Action |
|-----------|--------|
| `kitty_mod+c` | Copier dans le presse-papiers |
| `kitty_mod+v` | Coller depuis le presse-papiers |
| `kitty_mod+m` | Surligner `error` / `warn` dans l'output (marker) |
| `kitty_mod+h` | *(défaut)* Scrollback complet dans `less` (recherche `/`) |
| `kitty_mod+u` | *(défaut)* Insérer un caractère Unicode |
| `kitty_mod+r` | *(défaut)* Redimensionner la fenêtre active |
| `Ctrl+Shift+Alt+t` | *(défaut)* Renommer l'onglet courant |
| `Ctrl+Shift+F3` | *(défaut, ≥ 0.46)* Palette de commandes — toutes les actions |
...
| `kitty_mod+y` | Sélectionner un hash git → inséré dans le prompt |
| `kitty_mod+o` | Sélectionner une référence `fichier:ligne` → ouverte dans Zed |
...
**Exemple d'usage de `kitty_mod+y` :**
...
# kitty_mod+y surligne les hashes
...
!!! danger "Un nettoyage de `/tmp` casse le socket sans le faire planter"
    Symptôme : `kitty @ ls` (ou tout script basé dessus, comme `openz`)
    échoue avec `Failed to connect to unix:/tmp/kitty-PID … no such file
    or directory`, alors que le terminal fonctionne normalement et que le
    processus `kitty` correspondant tourne toujours.

    **Cause** : le fichier socket `/tmp/kitty-{kitty_pid}` a été supprimé
    après coup (nettoyage manuel de `/tmp`, script de ménage trop
    agressif…). Un socket Unix reste ouvert et actif en mémoire noyau même
    après la suppression de son fichier — `ss -xl` continue de l'afficher
    lié à ce chemin — mais plus aucun nouveau client ne peut s'y
    connecter puisque le chemin d'accès n'existe plus sur le disque.

    **Diagnostic rapide** :
    ```bash
    ss -xl | grep kitty          # le socket apparaît toujours "listen"…
    ls /tmp/kitty-$KITTY_PID     # …mais le fichier est introuvable
    ```

    **Remède** : il n'existe pas de commande pour « relier » un socket déjà
    lié à un chemin disparu — il faut redémarrer l'instance kitty
    concernée (fermer puis rouvrir la fenêtre, ou redémarrer la session)
    pour qu'elle rebind un socket avec un fichier présent.

    **Prévention** : ne pas vider `/tmp` à la main pendant qu'une session
    kitty est ouverte — préférer un redémarrage de poste, qui recrée
    proprement tous les sockets au passage.
...
`enabled_layouts Tall, Splits, *` — le layout **Tall** (défaut) divise la
fenêtre verticalement (une colonne principale large à gauche, les autres à
droite). **Splits** permet un découpage libre façon IDE (voir
`kitty_mod+d` et le [guide](guide.md#splits-decouper-lecran-facon-ide)).
`*` active tous les autres layouts ; `Ctrl+Shift+l` passe au suivant.
...
décalent l'alignement dans Starship ou btop.
...
3. Ouvre un nouvel onglet Kitty et se connecte à la VM choisie via
   **`kitten ssh`** — qui copie automatiquement le terminfo
   `xterm-kitty` et l'intégration shell sur l'hôte distant
   (indispensable sur Alpine, voir le
   [guide](guide.md#kitten-ssh-ssh-sans-friction))
...
| `scrollback_pager_history_size` | `10` | ~10 Mo consultables via `kitty_mod+h` |
| `enabled_layouts` | `Tall, Splits, *` | Tall par défaut, splits façon IDE |
