...
!!! note "Nuance sur `gnome-settings`"
    ...
    **immédiatement**, sans jamais passer par `session-checks`. Voir le
    ...
    [Post-installation — groupe desktop](post-installation.md#groupe-desktop-etapes-31-a-36).
...
|---|---|---|
| **Installer la CLI Claude Code** (`claude`) | `install_claude_terminal.sh` (étape 19) vérifie sa présence mais ne l'installe jamais — et son absence **arrête tout le pipeline** `make all` dès le groupe `cli`. Voir l'avertissement en tête de [Post-installation](post-installation.md). | `curl -fsSL https://claude.ai/install.sh \| bash` |
...
| **Authentification `pass-cli`** (`pass-cli login`) | `install_pass_cli.sh` (étape 17) installe/met à jour le binaire mais ne gère jamais l'authentification — obligatoire via login web si le compte est protégé par YubiKey/SSO | `pass-cli login` |
| **Authentification Proton Drive CLI** (`proton-drive auth login`) | `install_proton_drive_cli.sh` (étape 18) installe/met à jour le binaire mais ne gère jamais l'authentification | `~/.local/bin/proton-drive auth login` |
...
| **`systemctl restart libvirtd`** après la règle AppArmor de `vmforge` (étape 40) | Loggé comme instruction (`log_info "[NOTE] ..."`), jamais exécuté par le module | `sudo systemctl restart libvirtd` |
...
| **`check_updates.sh --apply`** | Détecte les outils upstream en retard mais ne met jamais à jour tout seul, par choix de conception (les `sudo`/`rm` doivent rester un geste volontaire) | `check_updates.sh --apply` (symlink `~/.local/bin`, étape 43) ou `~/alm_tools/postinstall/check_updates.sh --apply` |
