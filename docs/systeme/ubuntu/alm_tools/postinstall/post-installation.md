...
| 27 | `proton` | VPN via dépôt `protonvpn-stable-release` ; Mail, Pass et Authenticator via `.deb` officiel signé (SHA512), **hors dépôt APT** — jamais distribués via `repo.protonvpn.com` (voir [Architecture](architecture.md#audit-dintegration-complet-2026-07-12-methodologie-et-5-bugs-trouves)) | `dpkg -s` (les 4 paquets, tout-ou-rien) ; vérifications séparées pour le dépôt VPN (paquet + fichier sources) et pour chaque `.deb` (SHA512 fail-closed) |
