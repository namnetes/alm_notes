...
Invite de commandes multilingue avec ~225 lignes de configuration :
...
| Fichier | Rôle |
|---------|------|
| `settings.json` | ~290 lignes de configuration |
| `keymap.json` | Raccourcis clavier |
| `tasks.json` | Tâches de build et d'exécution |

**Modèles IA :** Claude Sonnet 5 (défaut), Claude Opus 4.8 et Claude
Haiku 4.5 disponibles (résumés/commits sur Haiku).
...
!!! info "Fichier stowé"
    `.tigrc` est déployé par Stow comme tous les autres dotfiles. Les
    couleurs de l'interface tig sont restaurées automatiquement à la
    réinstallation.
...
| Option | Valeur | Effet |
|--------|--------|-------|
| `color cursor` | `black white bold` | Ligne sélectionnée |
| `color status` | `black blue bold` | Barre de statut |
| `color title-blur` | `white default` | Titre des vues inactives |
| `color title-focus` | `white default bold` | Titre de la vue active |
...
|---------|------------------|------|
| `.config/claude/global_rules.md` | `~/.config/claude/global_rules.md` | Règles globales (Mermaid, écriture, git) — `.clauderc` (racine du dépôt, stowé en `~/.clauderc`) en est une copie identique |
| `.claude/CLAUDE.md` | `~/.claude/CLAUDE.md` | Standards de dev tous projets : Python, Shell, MkDocs, pioinit, devinit |
| `.claude/CLAUDE_Open.md` | `~/.claude/CLAUDE_Open.md` | Template pour projets Python / open (copié par `claude-open`) |
| `.claude/CLAUDE_Mainframe.md` | `~/.claude/CLAUDE_Mainframe.md` | Standards IBM z/OS : COBOL 6.5, JCL, Db2, CICS — copié par `claude-z` dans les projets mainframe |

!!! warning "Le `CLAUDE.md` à la racine du dépôt n'est pas déployé"
    Il documente le dépôt `alm_dots` lui-même et est exclu de Stow
    (`^CLAUDE\.md$` dans `.stow-local-ignore`). Les règles globales sont
    celles de `.claude/CLAUDE.md`.
